...
- Skill 版本：`2.5.2`；本文更新于 2026-09-21。
...
授权后先服从用户的分工选择。用户明确要求简单修改也用 OpenCode，就走轻量派工；用户要求由 Codex 直接修改，就直接完成。没有指定分工时，只有预计减少的 Codex 工作大于交接、观察、验收和返工成本才委派，直接处理不再局限于“一行修改”。业务语义、公共接口、安全边界、发布和合并等关键决定仍由 Codex 在用户授权范围内负责。
...
Codex 维护 `项目/工作区 + 连贯任务/工作流 + 模型/variant + 角色` 的 session 映射。上下文仍相关且健康时，连续任务和返工显式传入已有 `sessionId`，每轮使用新的 job/message ID。不是同一仓库里的所有需求都塞进一个 session；长会话按下文压缩或交接，但不丢失有效授权。并行 Worker 与独立 Verifier 使用不同 session，同一 session 不并发两个回合。
...
边界明确、风险低的任务默认是：**短简报 → 一个 Builder 完成修改和相关测试 → Codex 定向验收**。不自动增加 Scout、Verifier、完整台账或多轮计划确认；涉及资金、公开接口、安全或兼容契约时，仍按实际风险验收。

例如“修改一个标签，保持行为不变”：若用户要求 OpenCode 开发，只给相关文件、准确文案、保持项和必要检查即可。已有规则、配置和有效简报直接复用；已知测试命令也无需再派一个智能体执行。

### 复用 session，也管理上下文

续用前利用已有返回中的最近一次输入用量（包含缓存）和已核实的模型上下文上限，不额外轮询、不读取完整历史。输入达到上限的 **50%**、积累大量无关历史或反复遗失约束时，检查继续复用、压缩还是交接。50% 是可调整的检查触发值，不是经过性能证明的最优值，也不是强制失败线。

运行时确实支持压缩才使用其真实能力；否则等旧写者停止后，用新 session 接收“任务要求、授权、模型、工作区基线与当前状态、已通过检查、剩余工作、返工计数”的简短交接。新 session 不重新扫全仓，不清零任务计数。用量或上限未知时记为 `null`，根据上下文相关性判断，不编造数字或压缩 API。

### 按整个任务控制返工

集中发送已知问题，避免每发现一个问题就中断正在安全执行的 Worker。整个任务累计 **两次未成功的主控要求的实现返工** 后，先诊断共同原因，再决定缩小任务、重写简报或在用户允许的分工内接手。不同问题、不同 session 都不能重置计数。

初次实现、交付前普通自修复和仅补报告不算这两次。触发检查也不代表接受缺陷或强行取消仍有进展的任务；范围、安全或数据风险需要立即停止时不必等待两次。

### 慢任务持续跟进，不因等待超时重发

使用已有的 `opencode_wait`、`opencode_check` 和任务查询工具监控，不增加常驻监控服务。单次等待受宿主超时和进度回报间隔限制，**不代表任务只能运行这么久**。

- `accepted` / `running`：持续等待同一个 `jobId`，保留 session/message；执行慢、暂时没有输出或多次等待超时，都不重新派工。
- 进度检查点：派工时按任务阶段或已知工具耗时设置；没有依据时默认 5 分钟，允许调整。这是诊断时间，不是终止时间。到点查看相关工具、测试或近期事件，记录依据和下次检查时间；已返回的信息足够就不额外查询。
- 怀疑卡住：错误、待答问题或资源异常可提前诊断；重复的 `running`、心跳或进程存在不能单独证明有进展，也不能据此不断推迟检查。安静的长测试有正常执行依据就继续等；无法恢复观察时说明阻塞并保留写入所有权，不盲目重派。
- `input_required`：处理实际问题或权限请求；超出授权时向用户说明缺失信息，不盲目等待或重试。
- `unknown` / 断线：先恢复原任务状态；`completed` 后才读取结果并验收。

普通问题等 Worker 交付后集中反馈。只有用户要求停止/改变任务、实际越界或安全风险、已诊断必须停下才能恢复的阻塞，或明确的硬资源限制，才取消；先记录原因与依据，再调用真正的取消工具，并确认旧工具/进程已停，才能更换写者。

必要任务仍在运行时，主控继续跟进，不提前结束回复。用户要求暂停/后台运行，或确实无法继续观察时，应报告最后状态、任务句柄和未完成事项，不宣称完成，也不承诺不存在的自动唤醒。这些是 Skill 行为规则，不是 MCP 强制调度器。

### 失败重试要分类、有界

观察超时或断线先恢复原任务，提交结果未知时不能直接重发。确认旧任务及相关工具停止，或确认从未提交后，才判断是否可重试；先核对部分修改和外部副作用，防止重复执行。

- 临时基础设施故障：初次派工之外，整个任务默认最多自动重派 2 次；遵守服务端重试间隔，逐次延长等待，用户更严格的限制优先。
- 参数或配置错误：修复明确原因后再继续；权限拒绝不能绕过，代码或测试缺陷走实现返工流程。
- 次数耗尽或原因不明：停止自动重派、保留成果并诊断。有实际恢复变化或必要用户输入后才可据此记录新的尝试理由，不能换 job/session 或换个错误名称就清零计数。

正常等待不占重派次数；进度检查点不会补充次数。计数和下次检查时间沿用现有回执保存。该限制约束主控重派，不能强制限制提供商或工具内部的重试。

### 复用可信验收证据

Codex 检查原始需求覆盖、实际 diff、关键失败路径和测试断言。已观察到、且代码/测试/环境依赖相符的真实执行记录可以复用，不因换了审核者再跑一遍。修改影响哪些检查就补哪些；影响未知时先核实，项目和用户要求的最终检查仍必须执行。Worker 自写的“全部通过”不等于可信执行记录。

观察使用有界等待，超时先恢复已有任务，不盲目重派。正常回报保留结论、覆盖、路径和检查证据，不拉整份会话；截断只补缺失部分。

### 区分 Token、耗时和费用

沿用现有回执或台账，按用户目标记录 Codex 用量、双方总用量或耗时；不增建监控系统。模型和 `variant` 保持用户选择，不为了提速偷偷降低推理档位。

Codex 与 Worker 分开记录当前任务的输入、输出、推理、缓存和耗时，不能把长期 session 的累计量算成单任务用量。按提供商字段口径避免缓存/推理重复相加；不同模型的 Token 也不是完全等价单位。未知数据保留 `null`，不为凑指标频繁查询。软时间检查点用于判断进度，不等于超时杀任务。

这些规则的目标是减少交接和重复劳动；**规则、离线测试通过不证明实际更快或更省 Token**。需要固定任务、模型、环境和验收条件做对照；Token 计数也不等于订阅额度或账单。
...
上述 2026-09-18 的两次真实 MCP stdio 进程测试分别覆盖默认/紧凑模式，使用本机合成 HTTP 服务完成 `run → wait → job_get`；工具文档与目录校验也通过。这里“真实 stdio”不等于真实模型：**该组验证没有调用模型、连接 AWS，或做 token 节省对照实验**。
...
Skill 离线工具与规则连接测试 **108 项通过**；另外准备的 **108 个行为场景尚未作为完整评测集执行**，二者不能混为一谈。v2.5 的 8 个效率场景、v2.5.1 的 4 个等待/取消场景及 v2.5.2 的 4 个诊断/重试场景均待执行，本次未进行真实模型性能对照。可自行复验：
