...
/** Return a message view without reasoning parts; leave opaque payloads alone. */
export function filterMessageReasoning<T>(response: T): T {
  const message = response as any;
  if (!Array.isArray(message?.parts)) return response;
  // Filter transport parts only; business JSON and tool outputs are opaque.
  return { ...message, parts: message.parts.filter((part: any) => part?.type !== "reasoning") };
}

/** Extract text, tool summaries and usage metadata from a message response. */
export function formatMessageResponse(response: unknown): string {
  const r = filterMessageReasoning(response) as any;
