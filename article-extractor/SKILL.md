...
- Keep this skill self-contained; it does not require private paths, API keys, or bundled scripts.
- Required baseline: network access, a shell, `curl`, and `python3` for fallback extraction.
- Recommended tools: `reader` for Readability-style extraction, or `trafilatura` for robust Python-based extraction.
- Optional installers: `npm` for `reader`, and `pip`/`pip3` for `trafilatura`.
- On Windows, prefer WSL, Git Bash, or PowerShell equivalents; if shell snippets fail, adapt the commands while preserving the same extraction order.
- If a page requires login, paywall access, or heavy JavaScript rendering, say so and do not pretend the extraction is complete.
...
If no recommended tools are available, use basic curl + Python text extraction. This is less reliable but keeps the skill portable.
