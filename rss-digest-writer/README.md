...
- 用途：从 RSS、Reddit、YouTube、公众号 RSS 等来源聚合热点，由 Codex 完成筛选、去重、摘要和记录；默认用于 AI 热点发现，也支持用户指定其他话题。
- 适合场景：找近五天 AI 热点、追踪 AI 圈更新，或按用户指定领域整理 Reddit/YouTube/RSS/公众号来源。
- 技能类型：热点发现与摘要编排 skill；默认 AI source pack，也可通过用户指定 feed/source pack/网页补抓扩展到其他领域。
...
- 使用示例：`用 $rss-digest-writer 整理近五天 AI 圈热点`；`用 $rss-digest-writer 调研近 5 天半导体行业热点`。
- 迁移注意：不要提交 `__pycache__/`、`*.pyc`、临时输出 JSON、个人 RSS 列表或私有来源。

## English

- Purpose: Aggregate trends from RSS, Reddit, YouTube, WeChat RSS, and similar sources, then let Codex deduplicate, rank, summarize, and archive the findings. It defaults to AI trends but can switch to another user-specified topic.
- Best for: Finding recent AI trends by default, monitoring AI updates, or summarizing another user-specified topic from Reddit/YouTube/RSS/WeChat RSS signals.
- Skill type: Trend discovery and digest orchestration. It is not a general news crawler or a long-form writing skill. Use the AI source pack by default; use user-provided feeds/source packs or web fallback for non-AI topics.
...
- Example prompt: `Use $rss-digest-writer to summarize AI trends from the last five days`; `Use $rss-digest-writer to research semiconductor trends from the last 5 days`.
